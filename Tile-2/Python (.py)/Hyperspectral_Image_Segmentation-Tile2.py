...
for myFile in files:
    ...
    # Detecting bad bands if the ratio of non black pixels are less than 0.2% of the enitre image and neglecting them
    rows ,columns = image.shape
    if (cv2.countNonZero(image) < (0.002 * rows * columns )):
        continue
...
# ## 2. Dimensionality Reduction
# 
# We use Principal Component analysis to reduce the dimension of the remaining 176 bands into just 5 principle components. This is done using the Incremental PCA function imported from the Scikit-learn library. Incremental PCA is chosen because of the large number of features in the input data and would work well running it in small batches.
# 
